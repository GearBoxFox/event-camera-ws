...
class SharedBufferProducer:
    def __init__(self, h5file: Path, shared_ev_buffer: SharedEventBuffer):
        ...
        assert h5file.name.endswith('.h5') or h5file.name.endswith('.hdf5')
        self.h5f = h5py.File(str(h5file), 'r')
                
        # ADDED CODE
        # check if the file is the newer HDF5 format or not
        if h5file.name.endswith('.hdf5'):
            self.events = self.h5f['CD']['events']
        else:
            self.events = self.h5f
        ...
        self.idx_0 = 0
        self.num_events = self.events['t'].size
    ...
    def write_to_shared_buffer(self):
        ...
        if idx_1 > self.idx_0:
            read_events = Events(
                    self.events['x'][self.idx_0:idx_1],
                    self.events['y'][self.idx_0:idx_1],
                    self.events['p'][self.idx_0:idx_1],
                    self.events['t'][self.idx_0:idx_1])
